  ...
  async sendBookingNotificationToMerchant(booking, service, store, staff) {
    try {
      ...
      const templateData = {
        ...
        bookingLink: `${process.env.MERCHANT_DASHBOARD_URL || 'https://dashboard.example.com'}/bookings/${booking.id}`,
  ...
  async sendBookingConfirmationToCustomer(booking, service, user, store, qrCode) {
    try {
      ...
      const templateData = {
        ...
        qrCode: qrCode || '',
        bookingLink: `${process.env.CUSTOMER_PORTAL_URL || 'https://portal.example.com'}/bookings/${booking.id}`,
        storeInfo: {
