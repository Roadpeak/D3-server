...
const csrfProtection = (req, res, next) => {
  ...
  const csrfExemptPaths = [
    ...
    '/api/v1/staff',                    // Staff management (JWT authenticated)
    '/api/v1/branches',                 // Branch management (JWT authenticated)
    '/api/v1/categories',               // Category management (JWT authenticated)
    '/api/v1/transactions',             // Transaction endpoints (JWT authenticated)
    '/api/v1/follows',                  // Follow endpoints (JWT authenticated)
    '/api/v1/likes',                    // Like endpoints (JWT authenticated)
    '/api/v1/forms',                    // Form endpoints (JWT authenticated)
    '/api/v1/form-fields',              // Form field endpoints (JWT authenticated)
    '/api/v1/form-responses',           // Form response endpoints (JWT authenticated)
    '/api/v1/service-forms',            // Service form endpoints (JWT authenticated)
    '/api/v1/reviews',                  // Review endpoints (JWT authenticated)
    '/api/v1/merchant/',                // Merchant service endpoints (JWT authenticated)
    '/api/v1/request-service',          // Service request endpoints (JWT authenticated)
    '/api/v1/payments',                 // Payment endpoints (JWT authenticated)
    '/api/v1/hero',                     // Hero section endpoints (admin authenticated)
    '/api/v1/home-deals-stores',        // Home deals endpoints (admin authenticated)
    '/api/v1/store-gallery',            // Store gallery endpoints (JWT authenticated)
    '/api/v1/quotes',                   // Quotes endpoints (JWT authenticated)
    '/api/v1/subscriptions',            // Subscription endpoints (JWT authenticated)
  ];
